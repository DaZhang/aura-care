      ...
      <ScrollView scrollY className="h-[calc(100vh-450px)]">
        ...
        <View className="bg-white p-4 mb-2">
          <View className="flex items-baseline mb-2">
            ...
            <Text className="text-3xl font-bold text-[#E54B4B]">{product.price}</Text>
            <Text className="text-sm text-gray-400 line-through ml-2">¥{product.originalPrice}</Text>
          </View>
          <Text className="text-lg font-bold text-gray-900 mb-2">{product.name}</Text>
        ...
        {/* 产品特点 */}
        <View className="bg-white p-4 mb-2">
          <Text className="text-base font-bold text-gray-900 mb-3">产品特点</Text>
        ...
        {/* 香料配方 */}
        <View className="bg-white p-4 mb-2">
          <Text className="text-base font-bold text-gray-900 mb-3">专属配方</Text>
          <View className="space-y-3">
            {product.spiceInfo.map((spice, index) => (
              <View key={index} className="flex items-center p-3 bg-gray-50 rounded-xl">
                <View className="w-10 h-10 rounded-full bg-[#1D3A4C]/10 flex items-center justify-center mr-3">
                  <Text className="text-lg">{['🌿', '🌱', '🍃'][index]}</Text>
                </View>
                <View className="flex-1">
                  <Text className="font-medium text-gray-900">{spice.name}</Text>
                  <Text className="text-sm text-gray-500">{spice.effect}</Text>
                </View>
              </View>
            ))}
          </View>
        ...
        {/* 材质选择 */}
        <View className="bg-white p-4 mb-2">
          <Text className="text-base font-bold text-gray-900 mb-3">可选材质</Text>
          <View className="flex flex-wrap gap-2">
            {product.material.map((mat, index) => (
              ...
              >
                <Text className="text-sm text-gray-700">{mat}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View className="bg-white p-4 mb-4">
          <View className="flex justify-around">
            ...
            <View className="flex items-center">
              <Package size={20} color="#1D3A4C" />
              <Text className="text-sm text-gray-600 ml-1">急速发货</Text>
            </View>
            <View className="flex items-center">
              <Info size={20} color="#1D3A4C" />
              <Text className="text-sm text-gray-600 ml-1">专业客服</Text>
            </View>
          </View>
        </View>
      </ScrollView>
      ...
      <View className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 flex items-center gap-3">
        <View className="flex items-center gap-4">
          <View className="flex flex-col items-center" onClick={toggleFavorite}>
            <Heart size={24} color={isFavorite ? '#E54B4B' : '#999'} />
        ...
        <View className="flex-1 flex gap-2">
          <Button
            className="flex-1 border border-[#1D3A4C] text-[#1D3A4C] rounded-full py-3"
            onClick={handleCustomize}
          >
            立即定制
          </Button>
          <Button
            className="flex-1 bg-[#E54B4B] text-white rounded-full py-3"
